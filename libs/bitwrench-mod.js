 ...
 */
    ...
    if (y != "object" && y != "function") return y;
    if (baseTypeOnly == true)
      // so if undefind or anything but true
      return y;

    var r = y;
    try {
      r =
        x.constructor.name.toLocaleLowerCase() == y.toLocaleLowerCase()
          ? y
          : x.constructor.name; // return object's name e.g.
    } catch (e) {
      /*empty*/
    }
    if (r == "object") {
      if (x["_is_BW_HTMLNode"] == true) r = "BW_HTMLNode";
    }
    return r;
  };
 ...
 */
    if (["string", "array"].indexOf(_to(typeString)) == -1)
      // typeString must be a string or an arrag or strings
      typeString = "notValidType";

    if (_to(typeString) == "string") typeString = [typeString];
...
*/
    ...
    return typeString.indexOf(_to(a)) >= 0
      ? _to(trueValue) == "function"
        ? trueValue(a)
        : trueValue
      : _to(falseValue) == "function"
      ? falseValue(a)
      : falseValue;
  };
    ...
    if (_to(opts) == "object" && _to(dopts) == "object") {
      ...
      for (i in opts) {
        if (_to(opts[i]) == "object" || _to(dopts[i]) == "object") {
          var j;
          for (j in opts[i]) dopts[i][j] = opts[i][j];
        } else dopts[i] = opts[i];
      }
    }
    return dopts;
...
*/
    if (_to(a) != "array" || _to(b) !== "array") return [];
    return bw.arrayUniq(
      a.filter(function (n) {
...
*/
    ...
    //   if (_to(el) == "string") {
    //     // now its a string so we have choices..
...
*/
    ...
    if (els == []) bw.log("dom element not found");
 ...
 */
    var el = null;
    if (bw.isNodeJS() == false) {
      if (bw.DOMIsElement(htmldata)) el = htmldata;
      else {
      ...
      if (parentEl) {
        ...
        } else parentEl.appendChild(el); // put it last
      }
    }
    return el;
  };
...
*/
    ...
    c = c.map(function (x) {
      return bw.colorParse(x);
    }); // all colors will now be converted to bw RGB format
    ...
    var r = a - i;
    var _f = function (x) {
      return bw.mapScale(r, 0, 1, c[i][x], c[i + 1][x], { clip: true });
    };
 ...
 */
    ...
    var _fn =
      rnd == false
        ...
        : function (x) {
            return bw.clip(Math.round(x), 0, 255);
          };
...
*/
    ...
    if (_to(s) == "array") {
      // it could be a bwcolor type [c0,c1,c2,a,model]
      var p,
        df = [0, 0, 0, 255, "rgb"];
      for (p = 0; p < s.length; p++) df[p] = s[p];
      s =
        String(df[4]) +
        "(" +
        String(df[0]) +
        ...
        "," +
        String(df[3]) +
        ")"; //could use slice..join(",")
    }

    s = String(s).replace(/\s/g, "");
    var reT = /\s*(#|hsl|rgb|yuv|hsv){1}([a-f|A-F|0-9|,().\t ]*)/gim;
    var i,
      j = 0,
      x = reT.exec(s);
    if (_to(x) == "array" && x.length >= 3) {
      r[4] = x[1] == "#" ? "rgb" : x;
      if (x[1] == "#") {
        //parse one of these #rgb #rgba #rrggbb #rrggbbaa
        switch (x[2].length) {
          case 3: //#rgb
          case 4: //#rgba
        ...
        // its should be of form (c0,c1,c2) or (c0,c1,c2,alpha)
        r[4] = x[1].toLocaleLowerCase();
        if (x[2][0] == "(" && x[2][x[2].length - 1] == ")") {
          // parans are present
          var v = x[2].substring(1, x[2].length - 1);
          v = v.split(",");
          switch (
            v.length // valid entries are 3 or 4 components
          ) {
            case 3:
            case 4:
              for (i = 0; i < v.length; i++) r[i] = Number(v[i]);
              break;
            default:
              bw.logd("bw.parseColor bad input : " + s);
          }
 ...
 */
    var r = "#00000000";
    var ph = function (x) {
      var y = bw.clip(Math.round(x), 0, 255).toString(16);
    ...
    switch (c[4]) {
      case "rgb":
        ...
        if (!(format == "auto" && c[3] == 255)) r += ph(c[3]);
        break;
      case "hsl":
...
*/
    ...
    if (logdargs.indexOf("none") < 0) {
      var i = 0;
      var _a = [];
      for (i = 0; i < arguments.length; i++) _a.push(arguments[i]); //arguments, a reserved javascript keyword, is not a true array
    ...
    if (dopts["format"] == "text") {
      return _ld
        .map(function (x) {
          return x
            .map(function (y) {
              return bw.padString(y.toString(), 16, "left");
            })
            .join("\t");
        })
        .join("\n");
...
*/
    if (bw.isNodeJS() == true || typeof window != "object") return defaultValue;
    try {
 ...
 */
    ...
    try {
      ...
      var sh = function (u, b) {
        return b == true ? u : u.split(/#+/)[0];
      };
      var params = {},
        parts = sh(hs(url), allowHash).split("&");
      for (var i = 0; i < parts.length; i++) {
        var e = parts[i].split("=");
        if (!e[0]) continue;
        params[decodeURIComponent(e[0])] =
          _to(e[1]) == "string"
            ? decodeURIComponent(e[1].replace("#", "%23"))
            : true;
      }
      if (_to(key) == "undefined") return params;
      ...
      for (k in simpleDict) {
        s.push([
          encodeURIComponent(k) +
            "=" +
            encodeURIComponent(simpleDict[k].toString()),
        ]);
      }
      s = s.join("&");
    } else s = "";
 ...
 */
    ...
    return str
      .toString()
      .replace(
        new RegExp("[" + Object.keys(c).join("") + "]", "gm"),
        function (s) {
          return c[s];
        },
      );
 ...
 */
    ...
    return bw.html({
      ...
      a: {
        href:
          "data:image/svg+xml,<svg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 100 100'><text y='0.9em' font-size='90' style='fill:" +
          c +
          "'>" +
          iconStr +
          "</text></svg>",
...
*/
  bw.htmlIsVoidTag = function (tag) {
    return (
      " area base br col command embed hr img input keygen link meta param source track wbr ".search(
 ...
 */
    ...
    var isnu = function (x) {
      return bw.toa(x, ["null", "undefined"], true, false);
    }; // is x null or undefined
    var HTMLNode = function BW_HTMLNode() {
      this.t = "div";
      this.a = {};
      this.c = [];
      this.o = { tagClose: "auto" };
      this._is_BW_HTMLNode = true;
    }; //isBWHTMLNode is for IE compatiblity
    //function bwError  (v,x) {this.value=v; this.msg = (typeof x == "undefined") ? "error" : x;}

    var i,
      n = new HTMLNode(); // default html dict format
    switch (bw.to(x)) {
      case "null":
      case "undefined":
        n = "";
      ...
      case "object":
        [
          ...
          n[z[1]] = z[0] in x ? x[z[0]] : n[z[1]];
        });
        for (i in n) {
          // we only copy those fields we care about..
  ...
  bw.htmlEmit = function (htmlData, opts, state) {
      ...
      for (k in n.a) {
        ...
        if (k.search(/^on/) >= 0) {
          ...
          if (bw.to(v) == "function") {
            ...
            // not a function but some other type
            vr = "";
            try {
              vr = v.toString();
            } catch (e) {
          ...
          switch (k) {
            case "style":
              if (_to(v) == "string") vr = v.toString();
              else
                vr = bw
                  .makeCSSRule(["", v], { pretty: false })
                  .trim()
                  .replace(/^{/, "")
                  .replace(/}$/, "")
                  .trim();
    ...
    if (n.ntype != "BW_HTMLNode") {
      h.push(
        dopts.htmlEscContent
          ? bw.htmlSafeStr(n.node.toString())
          : n.node.toString(),
      );
    } else {
      // bw_HTMLNode

      if (n.node.t != "") h.push("<", n.node.t, _atr(n.node)); // add tag

      var tagClose =
        dopts.tagClose != "inherit" ? dopts.tagClose : n.node.o.tagClose;
      switch (tagClose) {
        ...
        default:
          ...
          if (bw.typeOf(n.node.c) != "array") {
            state.levelCnt++;
            state.levelMax =
              state.levelCnt > state.levelMax ? state.levelCnt : state.levelMax;
...
*/
    if (_to(listData) != "array") return bw.html(listData, options);

    return listData
      .map(function (x) {
        return bw.html(x, options);
      })
      .join("");
  };
 ...
 */
    ...
    var lc = listData.map(function (x) {
      return bw.html(["li", atri, x]);
    });
 ...
 */
    ...
    var tnorm = function (x) {
      x = bw.toa(x, "undefined", [], x);
      return bw.to(x) == "array" ? x : x.toString().trim().split(/\s+/gi);
    };
    var c = tnorm(classData);
    var ca = tnorm(classesToAdd);
    var cd = tnorm(classesToDel);
    return bw
      .arrayBNotInA(cd, c.concat(ca))
      .join(" ")
      .trim()
      .replace(/\s+/gi, " ");
  };
 ...
 */
    ...
    var ti = tabData.map(function (x) {
      return [
        "li",
        { class: "bw-tab-item", onclick: "bw.selectTabContent(this)" },
        x[0],
      ];
    });
    var tc = tabData.map(function (x) {
      return ["div", { class: "bw-tab-content" }, x[1]];
    });

    ti[0][1]["class"] = bw.classStrAddDel(ti[0][1]["class"], "bw-tab-active");
    tc[0][1]["class"] = bw.classStrAddDel(tc[0][1]["class"], "bw-show");

    dopts["atr"]["class"] = bw.classStrAddDel(
      dopts["atr"]["class"],
      "bw-tab-container",
    );
    dopts["tab_atr"]["class"] = bw.classStrAddDel(
      dopts["tab_atr"]["class"],
      "bw-tab-item-list",
    );
    dopts["tabc_atr"]["class"] = bw.classStrAddDel(
      dopts["tabc_atr"]["class"],
      "bw-tab-content-list",
    );
 ...
 */
    if (_to(data) != "array" || data.length < 1) return "";
    ...
    if (dopts.sortable == true) {
      ...
      if ("class" in dopts.th_atr)
        dopts.th_atr["class"] +=
          dopts.th_atr["class"].split(/[ ]+/).indexOf("bw-table-sort-xxa") < 0
            ? " bw-table-sort-xxa"
            : "";
      else dopts.th_atr["class"] = "bw-table-sort-xxa";
    } else {
    ...
    if (dopts["useFirstRowAsHeaders"]) {
      head = data[0]
        .map(function (x) {
          return _hs({ t: "th", a: dopts.th_atr, c: x });
        })
        .join("");
    ...
    for (; i < data.length; i++) {
      r = data[i]
        .map(function (x) {
          return _hs({ t: "td", a: dopts.td_atr, c: x });
        })
        .join("");
    ...
    //console.log(head,'\n',body);
    dopts.caption =
      dopts.caption == "" ? "" : _hs({ t: "caption", a: {}, c: dopts.caption });
 ...
 */
    var s = "";
    if (_to(data) !== "array") return s;

    var dopts = {
      atr: { class: "bw-accordian-container" }, // div for overall accordian
      atr_h: {
        onclick: "bw.DOMClassToggle(this.nextSibling,'bw-hide')",
        class: "bw-thm-light",
      }, // div wrapping each header
    ...
    s = data
      .map(function (x) {
        ...
        if (a["class"]) {
          a["class"] = show
            ? bw.classStrAddDel(a["class"], "", "bw-hide")
            : bw.classStrAddDel(a["class"], "bw-hide");
        } else a["class"] = show ? "" : "bw-hide";

        return (
          bw.html({ t: "div", a: dopts["atr_h"], c: [x[0]] }) +
          bw.html({ t: "div", a: a, c: [x[1]] })
        );
      })
      .join("");
 ...
 */
    ...
    if (false) {
      ...
      x.onreadystatechange = function () {
        if (x.readyState == 4 && x.status == "200") {
          callback_fn(prs(x.responseText));
        }
      };
      x.send(null);
    }
  ...
  bw.getJSONFile = function (fname, callback_fn) {
    return bw.getFile(fname, callback_fn, { parser: "JSON" });
  };
 ...
 */
    ...
    if (_to(message) != "undefined") bw.logd(String(message));
    return gBWTime;
  };
...
*/
    ...
    if (typeof delay != "number") delay = 250; // 250ms
    if (typeof maxReps != "number") maxReps = 1; // run 1 time.

    var _testFn = testFn;
    var _successFn =
      typeof successFn == "function" ? successFn : function () {};
...
*/
    ...
    var sortF =
      _to(sortFunction) == "function" ? sortFunction : bw.naturalCompare;
 ...
 */
    ...
    if (_to(item).substr(0, 4) != "html") return false; //something not right about this table element
    ...
    for (i = 0; i < cols.length; i++) {
      if (cols[i] == item) {
        ...
        if (dir) {
          ...
          } else bw.DOMClass(cols[i], "bw-table-sort-xxa", "bw-table-sort-upa");
        }
      } else {
        // its not the selected column so we clear the up / down arrow
 ...
 */
    if (fnID in _fnRegistry) delete _fnRegistry[fnID];
  };
 ...
 */
    fnID = String(fnID);
    if (fnID in _fnRegistry) return _fnRegistry[fnID];
    else {
      var _id = fnID;
      return _to(errFn) == "function"
        ? errFn
        : function () {
            bw.log(_id, "bw.funcGetById(): unregistered fn error");
          };
    }
  };
 ...
 */
    ...
    if (_to(numChars) != "number") numChars = l.length;
    ...
    if (s[s.length - 1] == " ") s = s.substring(0, s.length - 1) + "."; // always end on non-whitespace.  "." was chosen arbitrarily.
 ...
 */
    ...
    var _es = function (str) {
      return str.replace(/(?=[\\^$*+?.()|{}[\]])/g, "\\");
    }; // do escape of regex chars

    dopts["delims"] = bw.choice(
      dopts["docType"],
      {
        jsdoc: ["/**", "*/"],
    ...
    if (_to(r) == "array") {
      r = r.map(function (x) {
        return x.substring(
          dopts["delims"][0].length,
          x.length - dopts["delims"][1].length,
        );
      }); // this is an array of the contents of docStrings which can still be multiline in thier own right
      r =
        dopts["dropLeadin"] == true
          ? r.map(function (x) {
              return x.split(/[\n\r]/).map(function (y) {
                return bw.trim(y, "left") + "\n";
              });
            })
...
*/
    ...
    var a = s.replace(/^\s*(\/\*\*?)?|(\*\/)?\s*$/gi, ""); // remove the comment markers if still there "/** my comment */"" ==> "my comment"
    a = a.replace(/^\s*\**\s*/, ""); // remove any cruft at beginning of line " * @myParam {}....." ==> "@myParam {}....."
    if (a.charAt(0) == "@") {
      // if we have hit a @fieldname parameter we start parsing.
      ...
      if (x != null) {
        r["field"] = t(x[1]);
      } else return r; // didn't match... opt out here
      a = a.replace(e, "");
 ...
 */
    ...
    for (i = 1; i < a.length; i++) {
      ...
      if (l["field"] == "") {
        ...
        } else r[r.length - 1]["description"] += l["source"];
      } else r.push(l);
    }
    return r;
  };
 ...
 */
  ...
  bw.__monkey_patch_is_nodejs__ = new (function () {
    var _t = "ignore";
    this.set = function (x) {
      _t = _toa(x, "boolean", x, "ignore");
    };
    this.get = function () {
      return _t;
    };
 ...
 */
    ...
    var v = function () {
      return _to(value) == "function" ? value() : value;
    };
 ...
 */
    ...
    } else return data < l ? l : data > h ? h : data;
  };
 ...
 */
    ...
    var ms = function (z) {
      ...
      } else z = ((z - in0) / (in1 - in0)) * (out1 - out0) + out0;

      if (dopts["clip"]) z = bw.clip(z, out0, out1);
      return z;
    };

    if (_to(x) == "number") return ms(x);
    return x.map(ms);
  };
...
*/
    ...
    return x.length >= width
      ? x
      : new Array(width - x.length + 1).join(dopts["pad"]) + x;
  };
  // =============================================================================================
...
*/
    ...
    return (
      new Array(q[0]).join(dopts["pad"]) +
      x +
      new Array(q[1]).join(dopts["pad"])
    );
  };
 ...
 */
    ...
    var _rnd = function () {
      ...
      dopts.setType =
        ["int", "float", "number"].indexOf(dopts.setType) == -1
          ? "int"
          : dopts.setType;
...
*/
    ...
    var _rnd = function () {
      ...
      n =
        ((bw.hashFnv32a("start string", _cseed) & 0xffff) / 65536) *
          (rangeEnd - rangeBegin) +
        rangeBegin;
 ...
 */
    ...
    var p = "bw-theme-";
    var thm = ["l5", "l4", "l3", "l2", "l1", "d1", "d2", "d3", "d4", "d5"].map(
      function (x) {
        return p + x;
      },
    );
    var im = " !important";
    thm = thm.map(function (x, i) {
      return [
        x,
        [
          ["color", (i < 5 ? "#000" : "#fff") + im],
          ["background-color", c + im],
        ],
      ];
    });
 ...
 */
    ...
    var d = [
      ...
      [
        ".bw-table-stripe tr:nth-child(even)",
      ...
      [
        ".bw-tab-active",
        {
          /* padding-top:4px; padding-left:6px; padding-right:6px; padding-bottom:0;  */ "font-weight":
            "700",
        },
      ],
      [
        ...
        {
          cursor: "pointer",
          "font-weight": 700 /* border: 1px  solid #bbb; */,
        },
      ],
      [".bw-tab-content-list", { margin: 0, "padding-top": "0.0em" }],
      [".bw-tab-content", { display: "none", "border-radius": 0 }],
      [
        ".bw-tab-content, .bw-tab-active",
        { "background-color": "#ddd", padding: "0.5em" },
      ],
      "\n",
      //accordian
    ...
    [1, 2, 3, 4, 5, 6].map(function (x) {
      d.push([
        ".bw-h" + x,
        { "font-size": _r(3.2 * Math.pow(0.85, x + 1)) + "rem" },
      ]);
    });
    ...
    s +=
      d
        .map(function (x) {
          return rl(x, { pretty: dopts.pretty });
        })
        .join("\n") + "\n";
    ...
    //   if (appendToHead && document.getElementById(dopts["id"]) == null)
    //     // only append once
    //     h.appendChild(el);
    // }
    // if (dopts["exportCSS"])
 ...
 */
    ...
    var def = [
      ...
      },
      {
        css: [
          ...
          [
            ["table", "td", "th"],
            { "border-collapse": "collapse", border: "1px solid #111" },
          ],
          [["td", "th"], { padding: "4px" }],
          [
            ["div", "body", "button", "table", "input"],
    ...
    s = xs
      .map(function (y) {
        return bw.makeCSSRule(y, { pretty: false });
      })
      .join("\n");
 ...
 */
    ...
    if (_to(item).substr(0, 4) != "html") return false; //unable to set tab content
    document.gx = item;
    var i,
      j,
      index = 0;
    ...
    for (i = 0; i < cols.length; i++) {
      if (cols[i] == item) {
        // selected tab logic
        index = i;
        cols[i].className = bw.classStrAddDel(
          cols[i].className,
          "bw-tab-active",
        );
      } else {
        // unselected tab logic
        cols[i].className = bw.classStrAddDel(
          cols[i].className,
          "",
          "bw-tab-active",
    ...
    for (i = 0; i < item.parentNode.parentNode.children.length; i++) {
      if (
        item.parentNode.parentNode.children[i].className
          .trim()
          .split(/\s+/)
          .indexOf("bw-tab-content-list") >= 0
      ) {
        //were in the right child...
        for (
          j = 0;
          j < item.parentNode.parentNode.children[i].children.length;
          j++
        ) {
          if (
            item.parentNode.parentNode.children[i].children[j].className
              .trim()
              .split(/\s+/)
              .indexOf("bw-tab-content") >= 0
          )
    ...
    if (tcols.length <= 0) return false;
    ...
    for (i = 0; i < tcols.length; i++) {
      ...
      //bw.DOMClass(tcols[i],"bw-show","bw-show"); //tcols[i].style.display = "block";
        ...
        tcols[i].className = bw.classStrAddDel(
          tcols[i].className,
          "",
          "bw-show",
        );
      ...
      try {
        ...
        if (_to(replace) == "string" && c.indexOf(replace) == -1) {
          if (i == -1)
            ...
            if (replace.length > 0) c[i] = replace;
            else c.splice(i, 1);
          }
...
*/
    ...
    for (i = 0; i < elems.length; i++) {
      ...
      try {
        ...
        if (r) bw.DOMClass(x, className, "");
        else bw.DOMClass(x, className, className);
      } catch (e) {
        bw.log(e);
      }
    }
 ...
 */
    var v = {
      version: "1.2.16",
      about:
        "bitwrench is a simple library of miscellaneous Javascript helper functions for common web design tasks.",
 ...
 */
  ...
  var getArgs = function () {
    if (bw.isNodeJS() == false) {
      ...
      //pick up from URL
      if (bw.bwargs["enableUJURLArgs"] == "true") {
        //note string literal "true"
  ...
  var loadStyleBasics = bw.typeAssign(
    bw.bwargs["bw-load-style-basics"],
    "string",
    bw.bwargs["bw-load-style-basics"],
    "load",
  );
